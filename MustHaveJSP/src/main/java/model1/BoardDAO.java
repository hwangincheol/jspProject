...
public class BoardDAO extends JDBConnect {	//3번째 생성자로 활용
	...
	public BoardDAO(ServletContext app) {	//ServletContext app -> WEB-INF \ web.xml에 있는 값 활용
		super(app);
	...
	public List<BoardDTO> selectList(Map<String, Object> map) {
		List<BoardDTO> bbs = new Vector<>();	//결과(게시물 목록)를 담을 리스트 변수
		
		String query = "SELECT * FROM board";
		if (map.get("searchWord") != null) {
			query += " WHERE " + map.get("searchField") + " " + " LIKE '%" + map.get("searchWord") + "%'";	//조건이 있을때 where문 추가 if문
		}
